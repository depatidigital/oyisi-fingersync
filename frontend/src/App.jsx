...
export default function App() {
  ...
  const [addedUsers, setAddedUsers] = useState([]); // [{user, fingerprint_data}]
  const [loading, setLoading] = useState(false);
  const [importData, setImportData] = useState('');
  const [exported, setExported] = useState(null);
  const fileInputRef = useRef();
  const datFileInputRef = useRef();
      ...
      try {
        ...
        setAddedUsers(
          data
            ...
            .sort((a, b) => a.device_user_id - b.device_user_id)
            .map(item => ({
              user: {
                id: item.user_id,
                name: item.user_name,
  ...
  const handleSearch = async (e) => {
    ...
    setLoading(true);
    const res = await fetch(`${API_BASE}/users?q=${encodeURIComponent(value)}`);
  ...
  const handleAddUser = async (user) => {
    if (!addedUsers.find(u => u.user.id === user.id)) {
      ...
      try {
        const res = await fetch(`${API_BASE}/device-users/map`, {
          method: 'POST',
  ...
  const handleImport = () => {
    try {
      ...
      const newUsers = data.map((item, i) => ({
        user: {
          id: null,
          ...
          <div className="mb-4 sm:mb-6 relative">
            ...
            {(loading || searchResults.length > 0) && (
              <ul className="absolute left-0 w-full z-40 border border-gray-200 rounded mt-1 bg-white max-h-48 overflow-y-auto shadow-lg">
                ...
                {searchResults.filter(user => !addedUsers.find(u => u.user.id === user.id)).map(user => (
                  <li
                    key={user.id}
                ...
                {searchResults.filter(user => !addedUsers.find(u => u.user.id === user.id)).length === 0 && (
